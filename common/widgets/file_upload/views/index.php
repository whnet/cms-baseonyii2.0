...
<div class="per_upload_con" data-url="<?=$config['serverUrl']?>">
    <div class="per_real_img <?=$attribute?>" domain-url = "<?=$config['domain_url']?>">
        <?=!empty($inputValue)?'<img src="'.$config['domain_url'].$inputValue.'">':''?>
    </div>
    ...
    <input up-id="<?=$attribute?>" type="hidden" name="<?=$inputName?>" upname='<?=$config['fileName']?>' value="<?=isset($inputValue)?$inputValue:$config['imgUrl']?>" filetype="img" />
</div>